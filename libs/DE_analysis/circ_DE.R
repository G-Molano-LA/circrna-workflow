...
option_list <- list(
  ...
  make_option(c("-C", "--circ_info"), default=NULL, help="circular information file",
              metavar="FILE5"),
  make_option(c("-o", "--out"), default="circrna_DE.csv", help="output file",
              metavar="output")
  )
...
# 1. Load data
print("Loading data...")
metadata<-read.csv(opt$metadata, row.names = 1, # data frame containing information
          stringsAsFactors=TRUE)               #+ for each sample
# Make sure that in your metadata information there is a column names' condition'

gene_info <- read.csv(opt$gene, row.names=1) # data frame containing annotation
                                            #+ information for each gene
circ_info <- read.csv(opt$circ_info, row.names=1) # data frame containing annotation
                                            #+ information for each circular RNA
circrna_counts <-as.matrix(read.csv(opt$circ_counts, row.names=1))  # numeric matrix of circular counts
linear_counts <- as.matrix(read.csv(opt$linear_counts, row.names=1)) # numeric matrix of linear counts
